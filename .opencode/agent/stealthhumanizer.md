...
- Preferred CPA providers are available when local OpenCode config has `provider.cliproxyapi.options`: `cpa-gpt-55` (default `cx/gpt-5.5`) and `cpa-gemini-35-flash` (default `ag/gemini-3.5-flash-low`). OpenCode agents default to `9router/cx/gpt-5.5`.
